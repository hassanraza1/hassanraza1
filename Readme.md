# Muhammad Ain Ul Hassan Raza

**AI Automation Engineer** — Building intelligent systems that solve real Life/business problems.

---

## ⚙️ Expertise
- End-to-end WhatsApp, Web, Socail Media AI bot development (production-grade)
- Intelligent workflow automation (n8n, Make, Zapier)
- LLM integration & prompt engineering (Gemini, Anthropic, OpenAI)
- Conversational AI systems with persistent memory & session management
- Self-hosted & Cloud based AI infrastructure (Docker, Linux, PostgreSQL, Node.js, Hetzner)

---

## 🔴 Live in Production
| Project | Description | Stack |
|---|---|---|
| **Arik** | AI sales & support bot for WaltonMarket.com, live on WhatsApp | n8n · Gemini · PostgreSQL · WAHA · Docker - Redis - Hetzner|

---

## 🛠️ Tech Stack
![n8n](https://img.shields.io/badge/n8n-EA4B71?style=flat&logo=n8n&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Google Gemini](https://img.shields.io/badge/Gemini-4285F4?style=flat&logo=google&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)

---

## 🧠 Current Focus
- Agentic AI systems & multi-step reasoning workflows
- RAG pipelines & vector database integration
- Voice & Vision AI for real-world automation
- Large-scale workflow architecture & orchestration

---

## 📫 Contact
- 📧 ainulhassan594@gmail.com
- 🇵🇰 Based in Okara, Punjab, Pakistan
